...
# 🌟 [추가됨] 백그라운드 자동화를 위한 비동기 스케줄러 및 크론 트리거
...
# ==========================================
# 2.8. 🌟 [핵심] 전자동 위원회 소집 코어 로직
# ==========================================
async def run_automatic_committee():
    """스케줄러에 의해 무인으로 트리거되는 관제 함수"""
    channel = bot.get_channel(CHANNEL_ID)
    if not channel:
        print(f"⚠️ 에러: 채널 ID ({CHANNEL_ID})를 찾을 수 없습니다. .env 파일을 확인하세요.")
        return
...
# ==========================================
# 3. 디스코드 이벤트 및 명령어 처리 구역
# ==========================================
@bot.event
async def on_ready():
    print(f"✅ [시스템 가동] {bot.user} 관제탑 로그인 완료!")
    
    # 🌟 봇이 켜지면 백그라운드 스케줄러 가동
    scheduler = AsyncIOScheduler()
    # 테스트용 기동: 매 분 0초마다 자동 가동 (초단위 크론 설정)
    #scheduler.add_job(run_automatic_committee, CronTrigger(second=0))
    # 🌟 (수정 후): 매일 오전 9시, 오후 9시 정각에 자동으로 위원회 소집
    scheduler.add_job(run_automatic_committee, CronTrigger(hour="9,21", minute=0))
    scheduler.start()
    print("⏰ [스케줄러 가동] 무인 자동 관제 크론 작업이 등록되었습니다. (매 1분마다 수행)")

@bot.command(name="투표")
async def vote(ctx):
    """수동 명령어 호출 시에도 작동 가능하도록 연동"""
    global CHANNEL_ID
    CHANNEL_ID = ctx.channel.id # 수동 실행 채널로 타겟 채널 동적 전환
